...
from pyrogram.errors import ApiIdInvalid, ApiIdPublishedFlood, AccessTokenInvalid, FloodWait, AuthKeyUnregistered, SessionPasswordNeeded
...
app = Client(
    ...
    max_concurrent_transmissions=1,  # Reduced from 3
    in_memory=True,
    workdir="./sessions",  # Specify working directory for sessions
    no_updates=False  # Changed to False to ensure we receive updates
)
...
"""
        ...
        keyboard = InlineKeyboardMarkup([
            [
                ...
                InlineKeyboardButton("⏯️ Toggle Pause", callback_data="toggle_pause")
            ],
            [
                InlineKeyboardButton("🧹 Cleanup", callback_data="start_cleanup"),
...
async def stop_handler(_, message):
    ...
    try:
        ...
        for admin_id in processing_manager.download_queue:
            processing_manager.download_queue[admin_id].clear()
...
async def exit_handler(_, message):
    """Safely terminate the bot with confirmation"""
    try:
        # Check if bot is processing
        if processing_manager.is_processing:
            warn_text = "⚠️ **Warning:** Bot is currently processing files.\n\n"
        else:
            warn_text = ""
        ...
        confirm_keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ Yes, Exit", callback_data="confirm_exit"),
                InlineKeyboardButton("❌ No, Continue", callback_data="cancel_exit")
            ]
        ])
        
        # Send confirmation message
        await message.reply_text(
            f"{warn_text}⚠️ **Are you sure you want to exit?**\n\n"
            "This will:\n"
            "• Stop all running processes\n"
        ...
        await message.reply_text(f"❌ Error initiating exit: {str(e)}")

# Add callback handlers for the exit confirmation
@app.on_callback_query(filters.regex("^confirm_exit$"))
async def confirm_exit_callback(_, callback_query):
    """Handle exit confirmation"""
    try:
        status = await callback_query.message.edit_text("🔄 Initiating shutdown sequence...")
...
""")
        ...
        logger.error(error_msg)
        await callback_query.message.edit_text(f"❌ {error_msg}")

@app.on_callback_query(filters.regex("^cancel_exit$"))
async def cancel_exit_callback(_, callback_query):
    """Handle exit cancellation"""
    try:
        await callback_query.message.edit_text(
            "✅ Exit cancelled. Bot will continue running."
        )
    except Exception as e:
        await callback_query.message.edit_text(
            f"❌ Error cancelling exit: {str(e)}"
        )
...
async def main():
    try:
        ...
        for directory in ['temp', 'files/all', 'files/pass', ERROR_DIR, 'data', 'sessions']:
        ...
        # Initialize connection with values from .env
        ...
        if not success:
            ...
            print("\n❌ Connection failed. Please check logs and try again.")
            return
...
async def extract_handler(_, message: Message):
    ...
    try:
        ...
        progress_tracker = ExtractionProgress(app, message.chat.id)
        processing_manager.current_state = ProcessingState.EXTRACTING

        async def progress_callback(progress_data):
            progress_data['chat_id'] = message.chat.id  # Add chat_id to progress data
            await extraction_progress_callback(progress_data)

        def run_extraction():
            try:
                return process_archives_in_dir(ALL_FILES_DIR, PASS_FILES_DIR, progress_callback)
            except Exception as e:
        ...
        if success:
            ...
            await progress_tracker.message.edit_text(
                "❌ Some extractions failed. Check logs for details."
            )
...
@app.on_callback_query(filters.regex("^(fallback_yes|fallback_no)$"))
async def fallback_callback(_, callback_query):
    """Handle fallback confirmation response"""
    try:
        if callback_query.data == "fallback_yes":
            await callback_query.message.edit_text(
                "🔄 Running fallback extraction method...",
                reply_markup=None
            )
            # The actual fallback will be triggered by the terminal response
        else:
            await callback_query.message.edit_text(
...
async def extraction_progress_callback(progress_data):
    """Handle extraction progress updates and fallback prompts"""
    try:
        if progress_data['status'] == 'fallback_prompt':
            # Create confirmation keyboard
            confirm_keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Yes, Try Fallback", callback_data="fallback_yes"),
                    InlineKeyboardButton("❌ No, Skip", callback_data="fallback_no")
                ]
            ])
            
            await app.send_message(
                chat_id=message.chat.id,  # You'll need to pass this through
                text=f"{progress_data['message']}\n\nPlease confirm in the terminal as well.",
                reply_markup=confirm_keyboard
            )
            
        elif progress_data['status'] in ['fallback_running', 'fallback_complete', 
                                       'fallback_error', 'fallback_skipped']:
            await app.send_message(
                chat_id=message.chat.id,  # You'll need to pass this through
                text=progress_data['message']
            )
...
class BotManager:
    ...
    def request_exit(self, exit_code=0):
...
class TelegramConnection:
    def __init__(self, api_id, api_hash, bot_token, session_name):
        self.api_id = api_id
    ...
    async def _clear_sessions(self):
    ...
    async def _create_client(self):
        """Create a new client instance with proper configuration"""
        return Client(
            ...
            sleep_threshold=60,
            max_concurrent_transmissions=3,  # Changed back to 3 as in main.org.py
            in_memory=True
        )
    ...
    async def connect(self):
        ...
        try:
            ...
            global app  # Make sure app is available globally
            app = self.client
            ...
            wait_time = e.value
            if wait_time <= 60:  # If wait time is 1 minute or less
                logger.warning(f"Short FloodWait encountered ({wait_time}s), waiting...")
                await asyncio.sleep(wait_time)
                return await self.connect()  # Retry after waiting
            else:
                logger.error(f"Long FloodWait encountered ({wait_time}s)")
                return None
